...
- [x] **P4j（已实施，2026-07-03，commit 81acdb4，主审通过；遗留一行 nit：`economy._projected_score` 送达分写死 240，raw<90 时应为 `min(240, 120+raw*4//3)`，对手已交付且我 raw<90 的残局差额会算小，下轮顺手修）**：蹲守波次感知 + 稀缺冰竞争。10:37/10:38 现网两连败尸检（P4h N1 在场工作正常、败因均不在 P4i 范围）：局 1 732:750——economy 蹲守在 S09 白等 25 帧（对刷新波零感知），S13 的 r360/r400 刷新波（T_019/T_022 各 15 分）被领先对手 r423/428 吃掉、我 r441 到各差 18/13 帧，**不蹲即赢 762:750**（raw 105 差 5 分到 110 档=结算差 30）；局 2 722:767——对手大路资源流包圆 6 资源，我全场 **0 冰鉴**（S03 冰差 5 帧被截胡且无 fallback：S06 被慢边禁令硬杀、S07 被 B2 折扣让渡），鲜度 124:163 输 39 分。**改动 A**=蹲守重构（行军优先——负结论：蹲错点等波更亏；波次外推 `Task.refresh_round` 周期 40；只有波 ≤15 帧且脚下是候选节点才蹲；对手从未设卡放宽 P4e"落后不蹲"；终局差额模式：对手已交付差 ≤40 分可等满一波）；**改动 B**=冰鉴三小刀（0 冰边际 +6、B2 折扣 ETA 劣 ≤5 帧例外、慢边禁令 0 冰豁免）。实现文档 `docs/P4j-蹲守波次与稀缺冰竞争实现文档.md`（自包含：双局尸检+13 项单测+局 1 回归核心用例+三方回归）。与 P4i 无文件冲突可并行实施
