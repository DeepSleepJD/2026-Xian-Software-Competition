...
- [ ] **P4i（已定稿待实施，2026-07-03）**：小分队人手利用优化。现网四局实证：我方 8 支人手每局只用 2 支探路（r3 S02 + r32 S04，其中 S04 标记还过期作废，全系统净收益 ~2 帧），剩 6 支给"对手设卡"留的预备队在对手从不设卡的局里零目标，且 RUSH 阶段（390-450 帧起）规则禁止新提交小分队动作=未用即作废；对照组：赢我们 750:727 的对手 2696 用了 7/8 支（r2/r87/r191 远程清障 + r419 探路 S14 加速验核）。**批准范围=改动 1（自适应保留量：开局 4→150 帧未见对手设卡降 2→350 帧清仓 0→见过设卡粘滞 6 + weaken/scout 补 RUSH 闸）+ 改动 2（探路扩面：economy.current_plan 暴露任务目标+宫门 45+delay 特判窗口；时机收紧 ETA 40→25 杜绝过期；pending 8→16 防重复派）**；改动 3（小分队清障）/改动 4（远程先手削卡）未批不做。实现文档 `docs/P4i-小分队人手利用优化实现文档.md`（自包含：日志证据+规则依据+行级锚点+代码草图+9 项单测清单+三方回归；最大风险=设卡陪练局削卡预算 6→4-5，回退旋钮 SQUAD_RESERVE_EARLY 4→5→6）
- [ ] **P4j（已定稿待实施，2026-07-03）**：蹲守波次感知 + 稀缺冰竞争。10:37/10:38 现网两连败尸检（P4h N1 在场工作正常、败因均不在 P4i 范围）：局 1 732:750——economy 蹲守在 S09 白等 25 帧（对刷新波零感知），S13 的 r360/r400 刷新波（T_019/T_022 各 15 分）被领先对手 r423/428 吃掉、我 r441 到各差 18/13 帧，**不蹲即赢 762:750**（raw 105 差 5 分到 110 档=结算差 30）；局 2 722:767——对手大路资源流包圆 6 资源，我全场 **0 冰鉴**（S03 冰差 5 帧被截胡且无 fallback：S06 被慢边禁令硬杀、S07 被 B2 折扣让渡），鲜度 124:163 输 39 分。**改动 A**=蹲守重构（行军优先——负结论：蹲错点等波更亏；波次外推 `Task.refresh_round` 周期 40；只有波 ≤15 帧且脚下是候选节点才蹲；对手从未设卡放宽 P4e"落后不蹲"；终局差额模式：对手已交付差 ≤40 分可等满一波）；**改动 B**=冰鉴三小刀（0 冰边际 +6、B2 折扣 ETA 劣 ≤5 帧例外、慢边禁令 0 冰豁免）。实现文档 `docs/P4j-蹲守波次与稀缺冰竞争实现文档.md`（自包含：双局尸检+13 项单测+局 1 回归核心用例+三方回归）。与 P4i 无文件冲突可并行实施
