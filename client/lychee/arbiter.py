...
协议第 8 章"动作类别限制"：主车队动作/小分队动作/终局急策同帧各限 1 个，
超限 = INVALID_ACTION_CONFLICT（非法动作，累计扣分）；窗口出牌每个 contestId 限 1 张。
仲裁规则：按 priority 降序，每个类别只放行第一个，其余丢弃（策略层靠优先级竞争）。
类别划分依据附录 E 动作索引表：SQUAD_* 为小分队，RUSH_SPEED/RUSH_PROTECT 为急策
（BREAK_ORDER 只作 rushTactic 绑定、不独立发送），WINDOW_CARD 按 contestId 计，
...
def _category(action: dict) -> str:
    ...
    if name in _RUSH_ACTIONS:
        return "rush"
    if name == "WINDOW_CARD":
        return f"window:{action.get('contestId', '')}"
    return "main"
